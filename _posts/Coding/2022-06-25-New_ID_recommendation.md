...
def solution(new_id):
    new_id = new_id.lower()
    rule_2 = re.compile('[^a-z0-9-_.]')
    new_id = rule_2.sub('', new_id)
    while ".." in new_id:
...
> 3. while 문을 돌며 마침표(.)가 2번 이상 연속된 부분을 하나의 마침표(.)로 치환
> 4. ID 가 1글자 이상임을 확인하며 마침표(.)가 처음이나 끝에 위치한다면 제거
